...
\begin{center}
% \ifkanjifonts
%  {\Huge\lxpopfamily ＵＮＩＸ／ＬＩＮＵＸの基本操作}
% \else
...
プログラム等をつくるときに、UNIX上でよく使われるエディタに\vbindex{emacs}
がある。
...
Ubuntu系Linux (Ubuntu/Mint等)では，アプリケーションが \vbindex{deb} 形式と呼ばれる\nmindex{パッケージ}形式で配布されている。
パッケージ管理(ダウンロードやインストール，削除など)には\vbindex{apt}コマンドを使う。

RedHat系Linux (CentOS/Fedora/Vine等)では，
deb形式ではなく \vbindex{rpm}で配布されており，
パッケージ管理にはCentOSやFedoraでは\vbindex{yum}を，
Vine Linuxでは\vbindex{apt}を使う。

以下では，\verb|rpm|や\verb|apt|の利用方法は以下に簡単に説明する。

\verb|rpm|コマンドはダウンロードした個々の\verb|rpm|パッケージのインス
トールや削除に、\vbindex{apt-get}コマンドはパッケージのダウンロードか
...
てくれる。
パッケージ入手先を変更したいときは\vbindex{/etc/apt/sources.list}に登録する。
\begin{enumerate}
\item まず必ず行うおまじない:
...
% \end{verbatim}
%   \end{screen}
% 途中で２回、ライセンス条項の表示と、ライセンスを了解するかどうかの質問
