...
**「自分の研究結果を他人に興味をもって聞いてもらえるように最善を尽くすこと」**である。
...
5. 「えと」「あの」等、無駄なことは言わない。手で何かをいじる等無駄な動きはしない。
    - 聞き手に対する無駄な信号は、意思伝達のノイズとなる。

## わかりやすい説明にするための掟

1. 何の話をしているか，その目的を常に明らかに。
...
4.  常に論理的展開に注意。「つっこみ」の入らないような話し方を
...
- **いきなりパワーポイントに向かうのは厳禁!**
   - スライド作成に入る前に，話のストーリーを考え，その大雑把なページ構成をまず考える。
...
- 結果に関するページには，重要な結果や主張を簡潔に書く
- 図をたくさん。文字は少し。
   - 言いたいことは全て絵やグラフで説明できるようにする。
- 言いたいことは箇条書等で簡潔に。
   - 書いてあるキーワードをつないで読めば文章になるように書くのがコツ。
   - 説明のセリフをぶつぶつと口に出しながら作ると，話しやすい資料ができる。頭だけで考えようとしないこと。
- フォントや箇条書きのインデントの方法など，全体として統一感のあるものにする
    - スライドのテンプレートを必ず利用する
- アニメーションは最低限に。内容と関係ないイラスト等は使わない
- ページ番号をつける
   - コメントをもらうときに便利なので

# 図表の説明について

1. 結果のエッセンスを視覚的にわかりやすく示す上で，図表はとても重要。簡単なイラストでもよいので，図で説明をする工夫をすること。
2. 図表の番号(「図1」など)は不要
3. 説明の順番の原則は以下の通り
